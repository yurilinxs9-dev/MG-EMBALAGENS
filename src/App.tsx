...
const TestPDF = lazyRetry(() => import("./pages/TestPDF"));
...
const queryClient = new QueryClient({
  defaultOptions: {
    queries: {
      // Não retry em erros 4xx (RLS, tabela inexistente, etc) — falha rápido
      retry: (failureCount, error: any) => {
        const status = error?.status ?? error?.code;
        if (typeof status === "string" && /^4\d\d|^42|^PGRST/.test(status)) return false;
        return failureCount < 1;
      },
      staleTime: 60_000,            // 1 min — cache reutilizado entre navegações
      gcTime: 10 * 60_000,          // mantém cache 10 min
          ...
          <BrowserRouter>
            <Suspense fallback={<PageLoader />}>
              <Routes>
                ...
                {/* Rota de teste — sem auth, gera PDF de exemplo */}
