...
export default function Propostas() {
  ...
  const handleDownloadPDF = async (p: any) => {
    const itens = (p.proposta_servicos || []).map((s: any) => ({
      nome: s.servico_nome,
    ...
    await generatePDF({
      ...
      clienteCpfCnpj: p.cliente_cpf_cnpj || "",
      nota: p.observacoes || "",
      itens,
                ...
                <div>
                  ...
                  <div className="space-y-1">
                    {detalhe.proposta_servicos.map((s: any) => {
                      ...
                      const subtotal = (Number(s.valor_mensal) || 0) * qty;
                      return (
                        <div key={s.id} className="flex justify-between text-sm bg-muted p-2 rounded">
                          <span>{s.servico_nome} <span className="text-muted-foreground">× {qty}</span></span>
