...
const CATEGORY_COLORS: Record<string, string> = {
  sacolas_plasticas: "#3b82f6",
  sacolas_papel: "#dc2626",
  grafica: "#f59e0b",
  fitas_auxiliares: "#a855f7",
  outros: "#6b7280",
};

const CATEGORY_LABELS: Record<string, string> = {
  sacolas_plasticas: "Sacolas Plásticas",
  sacolas_papel: "Sacolas Papel",
  grafica: "Gráfica",
  fitas_auxiliares: "Fitas & Auxiliares",
  outros: "Outros",
};

const CATEGORY_ICONS: Record<string, React.ElementType> = {
  sacolas_plasticas: ShoppingBag,
  sacolas_papel: Package,
  grafica: FileText,
  fitas_auxiliares: Sparkles,
  outros: Box,
};
...
function categorizeServico(nome: string): string {
  ...
  if (lower.includes("sacola papel") || lower.includes("papel maquina") || lower.includes("papel máquina")) return "sacolas_papel";
  if (lower.includes("tag") || lower.includes("cartao") || lower.includes("cartão") || lower.includes("adesivo")) return "grafica";
  if (lower.includes("fita") || lower.includes("seda")) return "fitas_auxiliares";
  if (
    lower.includes("boca vazada") ||
    lower.includes("boca de palhaco") || lower.includes("boca de palhaço") ||
    lower.includes("alca fita") || lower.includes("alça fita") ||
    lower.includes("camiseta") ||
    lower.includes("alca cadeado") || lower.includes("alça cadeado") ||
    lower.includes("saco pp") || lower.includes("envelope") ||
    lower.includes("sacola lisa")
  ) return "sacolas_plasticas";
  return "outros";
}
...
export default function Vendas() {
  ...
  const categoriaStats = useMemo(() => {
    const stats: Record<string, { quantidade: number; valorTotal: number }> = {
      sacolas_plasticas: { quantidade: 0, valorTotal: 0 },
      sacolas_papel: { quantidade: 0, valorTotal: 0 },
      grafica: { quantidade: 0, valorTotal: 0 },
      fitas_auxiliares: { quantidade: 0, valorTotal: 0 },
      outros: { quantidade: 0, valorTotal: 0 },
    };
  ...
  const evolutionData = useMemo(() => {
    const months: { mes: string; sacolas_plasticas: number; sacolas_papel: number; grafica: number; fitas_auxiliares: number; outros: number }[] = [];
    const now = new Date();
      ...
      propostas
        ...
        .forEach((proposta) => {
          ...
          servicos.forEach((servico) => {
            ...
            if (cat !== "mes") {
              const qty = Number((servico as any).quantidade) || 1;
              monthData[cat] += (Number(servico.valor_mensal) || 0) * qty;
            }
          });
        });
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <Tabs value={periodo} onValueChange={(v) => setPeriodo(v as PeriodoFiltro)} className="flex-shrink-0">
            <TabsList className="h-8 flex-wrap">
              ...
              <TabsTrigger value="mes" className="text-xs h-7 px-2">Mês</TabsTrigger>
      ...
      <div className="grid gap-4 grid-cols-2 lg:grid-cols-4">
        ...
        <Card className="border-0 shadow-md">
          <CardContent className="p-4">
            <div className="flex items-center justify-between mb-2">
              <p className="text-[10px] sm:text-xs font-semibold uppercase tracking-wider text-muted-foreground">Ticket Médio</p>
        ...
        <Card className="border-0 shadow-md">
          <CardContent className="p-4">
            <div className="flex items-center justify-between mb-2">
              <p className="text-[10px] sm:text-xs font-semibold uppercase tracking-wider text-muted-foreground">Conversão</p>
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        <Card className="border-0 shadow-md">
          ...
          <CardContent className="px-2 sm:px-6">
            {barData.length > 0 ? (
              ...
              <div className="text-muted-foreground text-center py-12 text-sm">Nenhuma venda no período</div>
            )}
          </CardContent>
        </Card>
        ...
        <Card className="border-0 shadow-md">
          <CardHeader className="pb-2 px-4 sm:px-6">
            <CardTitle className="text-sm sm:text-base font-semibold">Distribuição por Categoria</CardTitle>
          </CardHeader>
          <CardContent className="px-2 sm:px-6">
            {pieData.length > 0 ? (
              <ResponsiveContainer width="100%" height={250}>
      ...
      <Card className="border-0 shadow-md">
        <CardHeader className="pb-2 px-4 sm:px-6">
          <CardTitle className="text-sm sm:text-base font-semibold">Evolução de Vendas (6 meses)</CardTitle>
        </CardHeader>
        <CardContent className="px-2 sm:px-6">
          <ResponsiveContainer width="100%" height={280}>
            <LineChart data={evolutionData}>
              ...
              <Line type="monotone" dataKey="sacolas_plasticas" name="Sacolas Plásticas" stroke={CATEGORY_COLORS.sacolas_plasticas} strokeWidth={2} dot={{ r: 3 }} />
              <Line type="monotone" dataKey="sacolas_papel" name="Sacolas Papel" stroke={CATEGORY_COLORS.sacolas_papel} strokeWidth={2} dot={{ r: 3 }} />
              <Line type="monotone" dataKey="grafica" name="Gráfica" stroke={CATEGORY_COLORS.grafica} strokeWidth={2} dot={{ r: 3 }} />
              <Line type="monotone" dataKey="fitas_auxiliares" name="Fitas & Auxiliares" stroke={CATEGORY_COLORS.fitas_auxiliares} strokeWidth={2} dot={{ r: 3 }} />
              <Line type="monotone" dataKey="outros" name="Outros" stroke={CATEGORY_COLORS.outros} strokeWidth={2} dot={{ r: 3 }} />
            </LineChart>
          </ResponsiveContainer>
        </CardContent>
      </Card>
